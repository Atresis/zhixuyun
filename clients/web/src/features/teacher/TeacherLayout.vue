...
const profileOpen = ref(false); const helpOpen = ref(false); const profileModal = ref(false); const dark = ref(false);
...
onMounted(async () => { await teacher.load(); syncForm(); });
...
function toggleTheme() { dark.value = !dark.value; document.body.classList.toggle("teacher-focus", dark.value); }
...
<template>
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand"><span class="brand-mark">序</span><div><strong>知序云</strong><small>实验教学平台</small></div></div>
      ...
      <div class="sidebar-foot">当前数据权限<br /><strong>本人授课课程</strong></div>
    </aside>
    <main class="main">
      <header class="topbar"><div class="crumb"><span>泉州信息工程学院</span><span>›</span><strong>{{ title }}</strong></div><div class="top-actions">
