...
const deleteTarget = ref<AdminCourse | null>(null);
...
function remove(course: AdminCourse) { deleteTarget.value = course; message.value = ""; }
async function confirmRemove() { if (!deleteTarget.value) return; saving.value = true; try { await store.deleteCourse(deleteTarget.value.id); deleteTarget.value = null; } catch (error) { message.value = (error as Error).message; } finally { saving.value = false; } }
function checkDuplicates() {
  const seen = new Set<string>();
  const duplicates = store.courses.filter((item) => { const key = item.code.trim().toLowerCase(); if (seen.has(key)) return true; seen.add(key); return false; });
  pageMessage.value = duplicates.length ? `发现 ${duplicates.length} 门课程编码重复，请先核对课程基础数据。` : `已检查 ${store.courses.length} 门课程，未发现重复课程编码。`;
}
...
<template>
  <section class="admin-page admin-course-page">
    <div class="admin-page-head"><div><h1>课程管理</h1><p>课程名称与课程编码用于基础查重，任课教师按学期单独安排。</p></div><div class="admin-head-actions"><button class="admin-secondary-button" type="button" @click="checkDuplicates"><ScanSearch :size="16" />检查重复课程</button><button class="admin-primary-button" type="button" @click="openCreate"><Plus :size="17" />新增课程</button></div></div>
    <div v-if="pageMessage" class="admin-info-strip admin-page-feedback"><ScanSearch :size="16" />{{ pageMessage }}</div>
    ...
    <AdminModal v-if="deleteTarget" title="删除课程" :subtitle="`${deleteTarget.name} · ${deleteTarget.code}`" @close="deleteTarget = null"><div class="admin-warning-strip"><ScanSearch :size="16" />删除课程会同时影响后续开课安排；存在教学班或业务关联时，服务端会阻止删除。</div><p v-if="message" class="admin-form-error">{{ message }}</p><template #footer><button class="admin-secondary-button" @click="deleteTarget = null">取消</button><button class="admin-primary-button" :disabled="saving" @click="confirmRemove">{{ saving ? '删除中' : '确认删除课程' }}</button></template></AdminModal>
  </section>
</template>
